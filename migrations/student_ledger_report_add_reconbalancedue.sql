-- Add reconbalancedue to the Student Ledger report RPC so the report shows
-- reconciliation-aware Collection/Fine/Balance totals (matches the dashboard
-- and class-wise tab pending logic).
...
CREATE OR REPLACE FUNCTION public.get_student_ledger_report(
    ...
    fineamount NUMERIC, balancedue NUMERIC, reconbalancedue NUMERIC,
    paynumber TEXT, paydate DATE, duedate DATE
) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY EXECUTE format($f$
        SELECT  fd.demfeeterm::TEXT, fd.demfeetype::TEXT,
                ...
                fd.balancedue::NUMERIC, fd.reconbalancedue::NUMERIC,
